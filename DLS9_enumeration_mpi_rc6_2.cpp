...
unsigned cur_launch_solved_wus = 0;
...
void controlProcess(int corecount, std::string ifilename );
int sendWU(std::vector<wu> &wu_vec, int computing_process_id);
void computingProcess( int rank );
void writeInfoOutFile(std::string control_process_ofile_name, std::vector<wu> wu_vec);
void writeCurStateSstream(std::vector<wu> wu_vec, std::stringstream &sstream_search_space_state_cur);
...
int main(int argc, char *argv[])
{
	...
	if (argc != 2) {
		std::cerr << "Usage : dls9_count_data";
		return 1;
	}
	...
	if (rank == 0)
		controlProcess(corecount, ifilename);
	else
	...
	{
	case 1:
		strcpy((char *)&FileName, "in\0");
		break;
	case 2:
		strcpy((char *)&FileName, argv[1]);
		break;
	default:
		cerr << "Command line error: must be \"dls.exe <file_name>\" or \"dls.exe\"";
		return -1;
	}
	...
	fwrite(&SquaresCnt, sizeof(SquaresCnt), 1, f);

	fclose(f);

    cerr << "Total squares count: " << SquaresCnt << endl;
    cerr << "OK";
    //getchar();*/
}
...
void controlProcess( int corecount, std::string ifilename )
{
	...
	std::fstream prev_state_file;
	std::fstream cur_state_file(ifilename);
	...
	while ( getline(cur_state_file, str) ) {
		...
		cur_wu.processing_time = -1.0;
		sstream >> cur_wu.processing_time; // get processing time if such exists
		sstream.str(""); sstream.clear();
		cur_wu.start_processing_time = cur_wu.end_procesing_time = -1.0;
		if (cur_wu.dls_number >= 0) {
			total_processed_wus++;
			cur_wu.status = SOLVED;
		}
		else
	...
	writeInfoOutFile(control_process_ofile_name, wu_vec);
	...
	for (int i = 0; i < corecount - 1; i++) {
		sending_result = sendWU(wu_vec, i+1);
		if (sending_result == -1) // number of tasks < number of computing processes (at the end of the experiment)
			break;
	}
	...
	while (total_processed_wus < wu_vec.size()) {
		// receive result
		MPI_Recv(&wu_index, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status);
		current_status = status;
		MPI_Recv(FileData, 11, MPI_CHAR, current_status.MPI_SOURCE, 0, MPI_COMM_WORLD, &status);
		...
		wu_vec[wu_index].status = SOLVED;
		total_processed_wus++;
		cur_launch_solved_wus++;
		
		// send back a WU
		sending_result = sendWU(wu_vec, current_status.MPI_SOURCE);
		
		writeInfoOutFile(control_process_ofile_name, wu_vec);
		// update two states - the current one and the rezerv one
		sstream_search_space_state_prev.str(""); sstream_search_space_state_prev.clear();
		sstream_search_space_state_prev << sstream_search_space_state_cur.str();
		sstream_search_space_state_cur.str(""); sstream_search_space_state_cur.clear();
		writeCurStateSstream(wu_vec, sstream_search_space_state_cur);
		
		double state_writing_time = MPI_Wtime();
		cur_state_file.open(ifilename, std::ios_base::out);
		cur_state_file << sstream_search_space_state_cur.str();
		cur_state_file.close(); cur_state_file.clear();
		std::cout << MPI_Wtime() - state_writing_time << " s for writing a state" << std::endl;
		prev_state_file.open(prev_state_file_name, std::ios_base::out);
		prev_state_file << sstream_search_space_state_prev.str();
		prev_state_file.close(); prev_state_file.clear();
	}
	
	std::cout << "control process finished" << std::endl;
...
{
	double percent_val;
	std::ofstream control_process_ofile(control_process_ofile_name.c_str(), std::ios_base::out|ios_base::app);
	control_process_ofile << std::endl << "***" << std::endl;
	control_process_ofile << "total WUs " << wu_vec.size() << std::endl;
	...
	control_process_ofile << "total_processed_wus " << total_processed_wus
		<< ", i.e. " << percent_val	<< " %" << std::endl;
	percent_val = double(cur_launch_solved_wus * 100) / double(wu_vec.size());
	control_process_ofile << "cur_launch_solved_wus " << cur_launch_solved_wus
		<< ", i.e. " << percent_val	<< " %" << std::endl;
	control_process_ofile << std::endl;
	control_process_ofile.close(); control_process_ofile.clear();
}
...
void writeCurStateSstream( std::vector<wu> wu_vec, std::stringstream &sstream_search_space_state_cur )
{
	...
	for (auto &x : wu_vec) {
		...
		sstream_search_space_state_cur << x.first_cells_known_values << " " << x.dls_number << " ";
...
{
	int result = 1;
	// find index of WU to process
	int first_non_started_wu_index = 0;
	for (;;) {
		if (wu_vec[first_non_started_wu_index].status == NOT_STARTED)
			break;
		first_non_started_wu_index++;
		if (first_non_started_wu_index == wu_vec.size()) { // all WUs are already have been started or are being in progress
			// number of tasks < number of computing processes (at the end of the experiment)
			std::cout << "no free WUs for sending" << std::endl;
			first_non_started_wu_index = -1;
			result = -1;
			break;
		}
	}
	std::cout << "first_non_started_wu_index " << first_non_started_wu_index << std::endl;
	
	if (result != -1)
		copy(wu_vec[first_non_started_wu_index].first_cells_known_values.begin(), wu_vec[first_non_started_wu_index].first_cells_known_values.end(), FileData);
	std::cout << "sending FileData " << FileData << std::endl;
	MPI_Send(&first_non_started_wu_index, 1, MPI_INT, computing_process_id, 0, MPI_COMM_WORLD);
	MPI_Send(FileData, 11, MPI_CHAR, computing_process_id, 0, MPI_COMM_WORLD);
	wu_vec[first_non_started_wu_index].start_processing_time = MPI_Wtime();
	wu_vec[first_non_started_wu_index].status = IN_PROGRESS;
...
void computingProcess( int rank )
{
	...
	for (;;) {
		...
		std::cout << "received wu_index " << wu_index << std::endl;
		if (wu_index == -1) {
			std::cout << "received stop-message" << std::endl;
			std::cout << "finalizing process " << rank << std::endl;
			break;
		}
		MPI_Recv( FileData, 11, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &status );
		std::cout << "received FileData " << FileData << std::endl;
		...
		std::cout << "sending wu_id "      << wu_index   << std::endl;
		std::cout << "sending FileData "   << FileData   << std::endl;
		std::cout << "sending SquaresCnt " << SquaresCnt << std::endl;
		MPI_Send( &wu_index,      1, MPI_INT,    0, 0, MPI_COMM_WORLD);
		MPI_Send( FileData,   11, MPI_CHAR,      0, 0, MPI_COMM_WORLD);
